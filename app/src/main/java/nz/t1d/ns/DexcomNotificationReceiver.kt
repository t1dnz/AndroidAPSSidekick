...
@Singleton
class DexcomNotificationReceiver @Inject constructor(
    @ApplicationContext context: Context,
    val ddr: DisplayDataRepository
) {
    private var context: Context = context
    private val TAG = "DexcomNotificationReceiver"
    ...
    fun listen() {
        Log.d(TAG, "Started Dexcom listener")
        // Listen to notifications update the fragment
        LocalBroadcastManager.getInstance(context).registerReceiver(
            notificationReciever, IntentFilter("DexcomNotification")
        )
    }

    fun stopListening() {
        Log.d(TAG, "Stopping Dexcom listener")
