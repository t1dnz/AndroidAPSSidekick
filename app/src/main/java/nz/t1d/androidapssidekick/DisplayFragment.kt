...
import nz.t1d.AndroidAPSSidekick.databinding.FragmentDisplayBinding
import nz.t1d.datamodels.GlucoseReading
import nz.t1d.datamodels.BaseDataClass
import nz.t1d.datamodels.Bolus
import nz.t1d.datamodels.Carb
...
class DisplayFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        binding.swiperefresh.setOnRefreshListener {
            val job = nightscoutPoller.fetchData()
    ...
    fun updateValues() {
        if (ddr.bglReadings.size > 0) {
            ...
            binding.bglReading.text = nf1dp.format(first.value)
    ...
    private fun createRecentEventRow(re: BaseDataClass): View {
        ...
        when (re) {
            ...
            is GlucoseReading -> {
                ...
                action.text = buildSpannedString {
                    bold { color(ResourcesCompat.getColor(requireContext().resources, R.color.teal_700, null)) { append("${nf1dp.format(re.value)}mmol/L ") } }
...
//        ag.start()
    ...
    fun directionImageId(bgl : GlucoseReading): Int? {
